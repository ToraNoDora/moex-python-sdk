...
#
class HistorySecuritiesBaseParams(BaseModel):
...
# 
class HistorySecurityParams(LangParams):
    sort_order: Optional[str] = "asc" # Доступные значения 'asc' и 'desc'.
    sort_order_desc:Optional[str] # (Устарело)
    # Порядок сортировки.
    # Доступные значения Пусто и 'DESC'.
...
class HistoryZcycParams(BaseModel):
    date: Optional[str] = "today" # Доступные значения 'asc' и 'desc'.
...
class HistoryBoardgroupsSecuritiesParams(HistorySecuritiesBaseParams):
    date: Optional[str] = "today" # (if need actions: date = 1900-01-01) Фильтр торгуемости инструментов: traded, nottraded или all
...
class HistoryTotalSecuritiesParams(LangParams):
    date: Optional[str] = "today" # Показать промежуточные итоги торгов (только для валютного рынка)
    start: Optional[int] = 0
    tradingsession: Optional[str]
...
class HistoryTotalSecurityParams(LangParams):
    from_at: Optional[str] = "1900-01-01" # Показать промежуточные итоги торгов (только для валютного рынка)
    start: Optional[int] = 0
